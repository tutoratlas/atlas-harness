  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `TutorAtlas cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installed.status === "unsupported"
            ? `TutorAtlas cannot install the relay client automatically on ${installed.platform}-${installed.arch}.`
